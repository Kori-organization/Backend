...
public class ReportCardDAO {
    ...
    public void updateSituation(int idStudent) {
        ...
        "WHEN COUNT(*) FILTER (WHERE situacao = 'Reprovado') > 0 THEN 'Reprovado' WHEN COUNT(*) FILTER (WHERE situacao = 'AprovadoR') > 0 THEN 'AprovadoR' ELSE 'Aprovado' END FROM (SELECT CASE WHEN g.grade1 IS NULL OR g.grade2 IS NULL THEN 'Em andamento' WHEN (g.grade1 + g.grade2) / 2 >= 7 THEN 'Aprovado' " +
        "WHEN g.rec IS NOT NULL THEN CASE WHEN ((g.grade1 + g.grade2) / 2 + g.rec) / 2 >= 7 THEN 'AprovadoR' ELSE 'Reprovado' END ELSE 'Recuperação' END AS situacao FROM subjects sub LEFT JOIN LATERAL (SELECT g.grade1, g.grade2, g.rec " +
